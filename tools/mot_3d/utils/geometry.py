...
def downsample(points, voxel_size=0.05):
    ...
    for i in range(points.shape[0]):
        ...
        sample_dict[(int(point_coord[0]), int(
            point_coord[1]), int(point_coord[2]))] = True
...
def PolyArea2D(pts):
    ...
    area = np.abs(
        np.sum((pts[:, 0] * roll_pts[:, 1] - pts[:, 1] * roll_pts[:, 0]))) * 0.5
    return area
...
def m_distance(det, trk, trk_inv_innovation_matrix=None):
    ...
    if trk_inv_innovation_matrix is not None:
        result = \
            np.sqrt(
                np.matmul(np.matmul(diff.T, trk_inv_innovation_matrix), diff)[0][0])
    else:
